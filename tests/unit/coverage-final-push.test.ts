...
describe("coverage: catalog.ts canonical location discovery", () => {
  it("discovers library from canonical location via skills.json (lines 101-102)", async () => {
    const { mkdtemp, writeFile, rm, mkdir } = await import("node:fs/promises");
    const { tmpdir } = await import("node:os");
    const { join } = await import("node:path");

    const tmpDir = await mkdtemp(join(tmpdir(), "caamp-test-catalog-"));
    const libDir = join(tmpDir, "agents", "skill-library");
    await mkdir(libDir, { recursive: true });

    // Create minimal skills.json
    await writeFile(join(libDir, "skills.json"), JSON.stringify({
      version: "1.0.0",
      skills: [],
    }));

    const saved = process.env["AGENTS_HOME"];
    const savedLib = process.env["CAAMP_SKILL_LIBRARY"];

    try {
      delete process.env["CAAMP_SKILL_LIBRARY"]; // disable env var discovery
      process.env["AGENTS_HOME"] = join(tmpDir, "agents");

      const catalog = await import("../../src/core/skills/catalog.js");
      catalog.clearRegisteredLibrary();

      // isCatalogAvailable should discover the canonical location
      const available = catalog.isCatalogAvailable();
      expect(available).toBe(true);

      catalog.clearRegisteredLibrary();
    } finally {
      if (savedLib !== undefined) process.env["CAAMP_SKILL_LIBRARY"] = savedLib;
  ...
  it("canonical location with index.js (line 98-99)", async () => {
    const { mkdtemp, writeFile, rm, mkdir } = await import("node:fs/promises");
    const { tmpdir } = await import("node:os");
    const { join } = await import("node:path");

    const tmpDir = await mkdtemp(join(tmpdir(), "caamp-test-catalog-idx-"));
    const libDir = join(tmpDir, "agents", "skill-library");
    await mkdir(libDir, { recursive: true });

    // Create an index.js that will fail to load (exercises catch on line 104)
    await writeFile(join(libDir, "index.js"), "invalid js syntax {{{{");
    ...
    try {
      ...
      // Should try index.js, fail, then fall through
      const available = catalog.isCatalogAvailable();
      // Won't succeed since index.js is invalid
