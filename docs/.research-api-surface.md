...
- **Module**: `src/types.ts`
- **Kind**: Interface
- **Description**: Top-level lock file structure at the canonical lock path (`getLockFilePath()`, default `~/.agents/.caamp-lock.json`).
- **Properties**:
...
- **Description**: Installs a skill from a local path: copies to the canonical location (`getCanonicalSkillsDir()/<name>`) and creates symlinks in each target provider's skills directory.
- **Parameters**:
...
- **Description**: Lists all skill names installed in the canonical directory (`getCanonicalSkillsDir()`).
- **Parameters**: None
...
- **Description**: Records a skill installation in the shared lock file (`getLockFilePath()`). Merges agent lists on re-install and preserves the original `installedAt` timestamp.
- **Parameters**:
...
- **Description**: Reads the shared lock file at `getLockFilePath()` (default `~/.agents/.caamp-lock.json`). Returns a default empty structure if the file doesn't exist or is corrupted.
- **Parameters**: None
